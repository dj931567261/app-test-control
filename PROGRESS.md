...
| 2026-05-19 | **CI 加跨客户端产物断言**：`.github/workflows/ci.yml` build/unit/smoke 之后新增一步，连跑 4 个客户端的 `setup` / `install-skills`，并断言：claude-code 写 `.mcp.json` 含 6 个 server；cursor 写 `.cursor/mcp.json` + 4 个 `.mdc` rule；codex 打印 TOML 含 `[mcp_servers.*]` + 写 `AGENTS.md` 4 个 section + `~/.codex/skills/<name>/SKILL.md` 4 份；claude-desktop 打印 JSON 片段含 `"mcpServers"`。本地全跑通后入 CI |
| 2026-05-19 | **README 残留收尾**：smart-qa 中立化后 README 示例还说 "→ AskUserQuestion"，改成"列编号让用户回 1,3,5 或 all"；仓库结构表 `scripts/` 注释补上 `prewarm` |
| 2026-05-20 | **opencode 客户端支持**：`setup-mcp.mjs` 加 `--client opencode` → 写项目根 `opencode.json`（schema 跟 .mcp.json 不同：`command` 是 `[bin,...args]` 单数组，`env` 字段叫 `environment`）；`install-skills.mjs --client opencode` 验证 `.claude/skills/` 在位（opencode 文档说 natively 兼容 Claude skill 路径，所以不写 `.opencode/skills/` 避免名字冲突）。`.gitignore` 加 `opencode.json`；`docs/CLIENTS.md` 加 opencode 节 + 矩阵第 5 行；`docs/INSTALL_FOR_AI.md` 客户端表加选项 5 + §4-E 步骤；CI 加 opencode artifact 断言。本地 57 单测 + 5 smoke 全过 |
