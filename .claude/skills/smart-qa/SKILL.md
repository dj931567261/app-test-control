...
**报给用户的形态**：用 `AskUserQuestion` 给出 3-7 条流的概要，让用户多选：

```
基于代码推断，发现这几条主要业务流，你想跑哪些？（multi-select）
☐ F1 - 启动 → Splash → 同意隐私 → 首页 (信号: SplashPage, /splash route, GoRouter)
☐ F2 - 登录 → 输入手机/密码 → 首页 (信号: LoginPage, /login, '登录' 按钮)
☐ F3 - KYC 全流程 → 7 步表单 (信号: 7 个 KycXxxPage)
☐ F4 - 查看订单 → OrdersPage (信号: OrdersPage, /orders route)
☐ F5 - 全部（含 F1-F4）
```

用户多选完，固化为本次的"测试计划"。如果用户说"还有别的吗"或要改 → 重新推断 + 二次确认（最多 2 轮，避免 ping-pong）。
...
2. **强制用户确认**：v1 必经 `AskUserQuestion`。原因：自动推断必有偏差，让用户改一次比错跑 10 步成本低。
